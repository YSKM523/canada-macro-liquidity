...
import { CA_QT_END_DATE } from './config';
...
export function runRobustness(
  ...
  // ---- regimes ----
  // vix_eod is always null for CA (no VIX series stored); null vix rows are excluded from vix regime.
  ...
  const regimes = {
    ...
    qt: regimeBreakdown(snaps, horizon, s => (s.date < CA_QT_END_DATE ? 'pre' : 'post')),
  ...
  return {
    ...
    caveats: [
      ...
      'vix regime axis is empty for CA (no VIX series stored) — all rows excluded from that breakdown',
    ],
  };
}
