...
// NOTE: The robustness/walkforward internals operate on BtSnap.spx.
// In production the worker maps tsx → spx at the boundary. Tests use spx directly.
...
describe('runRobustness', () => {
  ...
  it('null vix rows are silently excluded from vix regime (no crash)', () => {
    // CA: vix_eod is always null — simulate all-null vix
    const nullVixSnaps = bigSnaps(50).map((s: any) => ({ ...s, vix: undefined }));
    const result = runRobustness(nullVixSnaps, { iters: 50 });
    // vix regime should be empty (all excluded), but no crash
    expect(result.regimes.vix).toBeDefined();
    expect(Object.keys(result.regimes.vix).length).toBe(0);
    expect(Number.isFinite(result.strategy.ann_return)).toBe(true);
  });
});
