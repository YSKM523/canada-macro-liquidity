...
> ⚠️ **诚实定位(请先读这条)**:在 2017–2026 的真实数据上,本模型对未来 13 周 TSX 的预测力**不显著**(综合 Spearman IC ≈ **+0.01**(近乎 0),95%CI 跨 0,p ≈ **0.48**,独立样本仅 **37**)。它是一个**弱信号的宏观 regime / 风控仪表盘**,**不是择时工具、不保证跑赢大盘、不构成投资建议**。本项目刻意不粉饰回测、不编造 alpha。
...
| impulse(总资产 4 周扩缩 Δ4w) | 扩=顺风 | 0.06 † | +0.051 |
...
> **†** impulse 从「总资产水平 z」修正为「总资产 4 周变化 z(Δ4w,与 `qe_qt_regime` 同口径)」后,其 IC 由负转正(13w **+0.051**、4w +0.103、8w +0.097)。本次仅做**算法/文档一致性修复**,权重**仍保留在 0.06 地板**(沿用 2026-06-24 校准的结构性先验,未重新拟合)——这是有意的范围控制,不是漏改;待将来专门重新加权时再调整。
